<template>
  <div class="article_comment_list">
    <div class="comment_item" v-for="item in data.list" :key="item.id">
      <div class="avatar">
      ...
      <div class="comment">
        ...
        <div class="info">
          ...
          <span class="apply" @click="showApply(item.id, item.user.nickname)">回复 ({{ item.comment_count }})</span>
          <a-popconfirm
              title="确定删除这条评论吗"
              ok-text="删除"
              cancel-text="取消"
        ...
        <div class="sub_comment_list">
          <div class="sub_comment_item" v-for="sub in item.sub_comments" v-if="item.sub_comments.length" :key="sub.id">
            ...
            <div class="sub_comment">
              <div class="line">
                <span class="name">{{ sub.user.nickname }}</span><span class="content">{{ sub.content }}</span>
              </div>
              <div class="info">
          ...
          <div class="sub_comment_apply" v-if="state.parent_comment_id === item.id">
            ...
            <a-input class="comment_ipt" @keydown.ctrl.enter="apply" v-model:value="state.content"
                     :placeholder="'回复 @ ' + state.nickname"></a-input>
...
function showApply(parentID, nickname) {
