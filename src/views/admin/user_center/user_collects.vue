    ...
    >
      <template #cell="{column, record,index}">
        ...
        <template v-if="column.key === 'title'">
          <span class="article_title" v-html="record.title"></span>
        </template>
        ...
        <template v-if="column.key ==='abstract'">
          <div class="article_abstract">{{ record.abstract }}</div>
        </template>
...
const data = reactive({
  ...
  columns: [
    ...
    {title: '作者', dataIndex: 'user_nickname', key: 'user_nickname'},
...
function onFilter() {
  bTable.value.ExportList({tag: tag.value, key: category.value})
}
