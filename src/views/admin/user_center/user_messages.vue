<template>
  <div>
    <a-modal :title="title" v-model:visible="visible" class="chat_modal" width="40%">
      <div class="chat_body">
    ...
    <Table :columns="data.columns"
      ...
      <template #cell="{column,record}">
        <template v-if="column.key ==='send_user'">
          ...
          <span style="margin-left: 1rem">{{ record.send_user_nickname }}</span>
        </template>
        <template v-if="column.key ==='rev_user'">
          <img :src="record.rev_user_avatar" height="40" style="border-radius: 50%" alt="">
          <span style="margin-left: 1rem">{{ record.rev_user_nickname }}</span>
        </template>
      </template>
...
const data = reactive({
  list: [{
    "send_user_id": 2,
    "send_user_nickname": "zhangsan",
    "send_user_avatar": "/uploads/avatar/default.jpg",
    "rev_user_id": 1,
    "rev_user_nickname": "admin",
  ...
  columns: [
    ...
    // {title: '发送人', dataIndex: 'send_user_nickname', key: 'send_user_nickname'},
    // {title: '发送人头像', dataIndex: 'send_user_avatar', key: 'send_user_avatar'},
    {title: '接收人', dataIndex: 'rev_user', key: 'rev_user'},
    // {title: '接收人', dataIndex: 'rev_user_nickname', key: 'rev_user_nickname'},
...
async function applyModal(record) {
  ...
  if (record.send_user_id === store.userInfo.user_id) {
    // 我是发送方
    title.value = `与${record.rev_user_nickname}的聊天`
    user_id = record.rev_user_id
  }
  if (record.rev_user_id === store.userInfo.user_id) {
    // 我是接收方
    title.value = `与${record.send_user_nickname}的聊天`
