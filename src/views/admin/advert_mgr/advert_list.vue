  ...
  <div>
    <a-modal :title="editID ? '编辑广告' : '添加广告'" v-model:visible="data.visible" @ok="handleOK">
      ...
      >
        ...
        <a-form-item label="图片地址" name="image_url" has-feedback
                     :rules="[{ required: true, message: '请输入图片地址' ,trigger: 'blur'}]">
          <a-input v-model:value="state.image_url" placeholder="图片地址"/>
        </a-form-item>
        <a-form-item label="是否显示" name="is_show">
          <a-switch v-model:checked="state.is_show"/>
        </a-form-item>
        <a-form-item label="图片预览">
          <img :src="state.image_url" height="60" style="border-radius: 5px" alt="">
        </a-form-item>
      </a-form>
    </a-modal>
    ...
    >
      ...
      <template #cell="{column, record}">
        <template v-if="column.key === 'image_url'">
          <img :src="record.image_url" alt="" height="60" style="border-radius: 5px">
        </template>
...
const data = reactive({
  columns: [
    ...
    {title: '图片路径', dataIndex: 'image_url', key: 'image_url'},
...
function updateModal(record) {
  ...
  state.image_url = record.image_url
