  ...
  <div>
    <a-modal v-model:visible="data.modalVisible" title="添加用户" @ok="handleOk">
      <a-form
          ...
          autocomplete="off"
      >
        <a-form-item label="用户名" name="use_name" has-feedback
    ...
    <a-modal v-model:visible="data.modalUpdateVisible" title="编辑用户" @ok="update">
      ...
      >
        <a-form-item label="昵称" name="nickname" has-feedback
                     :rules="[{ required: true, message: '请输入昵称' ,trigger:'blur'}]">
          <a-input v-model:value="formUpdateState.nickname" placeholder="昵称"/>
        </a-form-item>
    ...
    >
      ...
      <template #edit="{record}">
        <a-button class="table_action update" @click="updateModel(record)" type="primary">编辑</a-button>
      </template>
      <template #delete="{record}">
        <a-button class="table_action update" @click="userRemove(record)" type="danger">删除</a-button>
      </template>
...
function onFilter() {
  ...
  bTable.value.ExportList({role: filter.value})
}
...
async function removeBatch(userIdList) {
  ...
  bTable.value.ExportList() // 更新完成刷新列表页
}
